...
흐름도(§6.6.2) — 텍스트 점역 모드(그래픽 점역 모드는 촉각 그래픽이라 본 파이프라인 범위 밖):
  ...
  ※ 도형 점형(@$R 등)은 어느 상자가 무슨 도형인지 앞단이 안 줘서 못 적는다 — 입력이 없는
    것이지 글리프를 몰라서가 아니다(디코드 표는 visual_braille 모듈 도크스트링).
...
#   "3칸에서 시작"(= 앞 빈칸 2)이다. `visual_drafts._OUTLINE_BASE` 와 같은 값이 됐다 —
#   그림·사진 갈래와 도표 골격 8종이 이제 한 칸에 선다.
_TYPE_NOTE_INDENT = 2     # 도서지침 3장 2절 4)(1)(2) L2368·L2384 유형 제시어 머리줄 "3칸에서 시작"
...
# ★ 유형 제시어와 쌍점은 **한 덩이로 주 안**이다(2026-09-08 대표 지시).
#   종전 `<!주>그림<!/주>:` 은 쌍점을 주 밖에 흘려 `,'그림,':` 로 나갔다.
#   「제작 지침」 §6.3.4(1)·제3장 제2절 4)(1) L2367-2369 "점역자 주표 ,' ,' 안에
#   '시각 자료 유형'을 적고, 쌍점과 원본 자료의 내용을 이어 적는다".
#   gold 실물도 같다 — `【점역자주】그림: ①담배모자이크병에 걸린 …`.
#   ⚠ **쌍점은 주 밖이다 — 규정대로 둔다**(2026-09-08 대표 지시 "조항 없는 자리는
#      강제하지 말라"). 「점자 도서 제작 지침」 제3장 제2절 4)(1)(2) L2367-2369·L2383-2385:
#      "점역자 주표 ,' ,' 안에 '시각 자료 유형'을 적고, **쌍점 "1과 원본 자료의 내용을
#      이어 적는다**". [예 3-19] 원본 BRF 도 `,'@["o5,'"1` 로 쌍점이 주 밖이다.
#      ★ gold 는 반대다 — 전수 실측(corpus/pages/braille 18,892쪽 · 주표 쌍 11,838건)에서
...
def _structure(ext: ExtractedContent, subtype: str) -> dict:
    ...
    if real:
        return real                      # 앞단이 준 값 — title 이 있으면 원본에서 관측된 제목이다
    st = structure_from_caption(ext.corrected_text or "", subtype) or {}
    # ★ 캡션에서 만든 구조의 `title` 은 **캡션 첫 줄**, 곧 점역자(LLM)가 쓴 글이다.
    #   원본에 인쇄된 제목이 아니므로 §6.3.3(1) 제목 자리에 못 앉는다 — `_head_lines` 참조.
    if st.get("title"):
        st["_llm_title"] = True
    return st
...
# `structure["title"]` 이 **원본에 인쇄된 제목**일 때만 §6.3.3(1) 제목 자리를 쓴다.
# ★ 2026-09-09 대표 QA(#794) — 점역자(LLM)가 쓴 글이 그 자리로 나가고 있었다. 주표가 없어
#   점자로는 **묵자(원본에 인쇄된 글자)로 읽힌다.** 원본에 없는 글이 제목처럼 서면 점역사가
#   원본과 대조할 수 없다. 조문이 그 자리를 원본 것으로 못박는다 —
#     「점자 자료 제작 지침」 §6.3.3(1) L3168
#       "시각 자료의 제목은 **원본 자료의** 위치와 상관없이 해당 시각 자료의 윗줄 5칸에 적는다."
#     같은 지침 §6.3.4(2)① L3177-3179
#       "**제목이 없는** 시각 자료를 설명하거나 생략할 경우 점역자 주표 안에 …
#        '시각 자료 유형: 추가 설명문'을 적고, 다음 줄에 원본 시각 자료에 포함된 내용을 적는다."
#   곧 제목이 없으면 그 자리를 **비우고** 설명은 주표 안 유형 제시어 뒤로 간다. 규정 예시
#   전수(25건)도 같다 — 유형어 줄 **앞 별도 줄**에 설명을 둔 예시 0건, 설명이 서는 자리는
#   전건 유형어 줄 뒤 같은 줄(자료지침 예6-1 · 도서지침 예3-25)이다.
#
# ★ 2026-09-10 정정(#794 잔여) — 종전 판정은 **종결어미+마침표**(`_is_summary`)였다. 그것은
#   글의 출처가 아니라 **문장 꼴**을 보는 것이라, 캡션 첫 줄이 명사구로 오면 LLM 이 지은 말이
#   그대로 제목 자리에 앉았다. 실측(storage/jobs 실행분 시각 캡션 252건 재생): 골격이 선
#   67건 중 **51건(76.1%)** 이 제목 자리로 나갔고 전건이 캡션 첫 줄이다 —
#   `고려 중앙 통치 조직도` · `구성원 3명` · `벤 다이어그램` · `맥락의 갈래` · `가계도.`.
#   판정 기준을 꼴이 아니라 **출처**로 바꾼다: 앞단이 준 title 이면 원본 제목, 캡션에서
#   만든 title(`_llm_title`)이면 설명이다. 캡셔너 프롬프트가 첫 줄을 "제목이 글자로 있으면
#   제목을, 없으면 … 관계를 문장으로" 라고 시켜 둘을 한 자리에 섞어 놓기 때문에, 꼴로는
#   영영 못 가른다. 실제 인쇄 제목은 MinerU 가 `caption` 요소로 따로 잡아 본문으로 나간다
#   (`result_builder._link_captions`) — 이 자리에서 또 적을 것이 아니다.
# ⚠ 이 함수는 **쌍점·주표 위치를 안 건드린다.** 그 축은 원장 C-D4 로 자문 대기다.


def _head_lines(structure: dict) -> tuple[list[str], list[int]]:
    """골격 머리 → (줄, 줄별 들여쓰기). 원본 제목일 때만 5칸 제목 줄을 세운다."""
    title = (structure.get("title") or "").strip()
    if not title:
        return [_TYPE_NOTE_LINE], [_TYPE_NOTE_INDENT]                        # §6.3.4(1)
    if structure.get("_llm_title"):                                          # 캡션 첫 줄 = 설명
        return [f"{_TYPE_NOTE_LINE} {title}"], [_TYPE_NOTE_INDENT]           # §6.3.4(2)①
