...
def two_col_mode() -> str:
    ...
    **열이 둘이라고 풀어 적으라는 조문은 없다.** 그런데 우리는 `max_col == 2`면 무조건
    ...
    ★★ 2026-09-09(#793 · 원장 C-30 닫음) — **기본을 `table_grid` 로 뒤집었다.** 대표 지시
      "표는 기본 선택안이 테두리와 구분선이 있는 유형이어야 해". 위 실측이 이미 같은 쪽을
      가리키고 있었고, 조항도 §3.1.1(1)① 이 먼저다. 실측(경계 파일 실물 표 76개 · 중복
      제거): 기본 선택이 `테두리+구분선` 43 → **68**, `테두리만` 25 → 0, `번호 체계` 7 유지.
      `번호 체계` 는 §3.1.1(1)③ 이 정한 별도 갈래라 그대로 둔다(①②로 안 되는 문장형 표).
      되돌릴 때는 `TABLE_TWO_COL=linear`.
    """
    return "linear" if os.environ.get("TABLE_TWO_COL") == "linear" else "table_grid"
...
# ★ 앞 표지는 `표` 나 닫는 대괄호를 **반드시** 요구한다. 종전 `^표?\s*시작` 은 `표` 가
#   선택이라 "시작"으로 시작하는 **정상 주**의 첫 낱말을 통째로 먹었다
#   ("시작 지점을 표시함" → "지점을 표시함" · "시작점의 값이 0임" → "점의 값이 0임").
_TN_HEAD_WORD_RE = re.compile(r"^(?:표\s*시작|시작\s*\])\s*[\],.:·]?\s*")
