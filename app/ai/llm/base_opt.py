...
# 표(L7) — 재구조화 4-3(#755). 표의 `tn_text` 는 점자에도 묵자에도 안 실린다.
#   · 점자: `table_braille` 이 tn 을 쓰는 갈래는 `parsed_rows is None and "|" not in text`
#     하나인데(`table_braille.py`), `_table_tags` 가 본문이 있는 한 언제나 구조 태그를
#     돌려줘 그 갈래가 열리지 않는다(1열 표도 2026-08-29 N031 수정으로 격자로 간다).
#   · 묵자: `pipeline._print_src` 는 초안이 있으면 선택 초안을 쓰고 tn 은 초안이 없을 때만 본다.
#   실측(dev·val 경계 1,131쪽 전수 · 표 406개 · 같은 커밋 두 팔): 406/406 이 초안 5안을 갖고
#   **두 팔의 점자가 406/406 바이트 동일**하다. 남는 효과는 FE 「대체 텍스트 선택」 화면 문구
#   하나뿐인데, LLM 팔은 그 문구의 126/406(31.0%) 이 손볼 자리였다 — 처리 불가 10 ·
#   프롬프트 표지·마크다운 유출 37 · 문장 미종결 96 · 우리 처리 절차 1인칭 12(합집합).
#   규칙 팔은 406/406 이 `표. N항목 M행 표임.` 형식을 지킨다.
# 수식(L6) — 재구조화 4-2(#766). 코퍼스 1,131쪽 전수 두 팔에서 LLM 이 이기는 자리가 없다.
#   정방향 CER 은 규칙 팔이 양쪽 net-positive(dev 35.046→35.045% · val 36.719→36.705% ·
#   총편집 −259셀)이고, 이물질·검토 플래그·죽은 쪽은 무차, 외부 호출은 95→0 이다.
#   ★ **"양쪽 net-positive" 규칙의 세 번째 예외다 — 칸수 악화를 알고도 채택한다.**
#   LLM 팔이 「수식의 내용을 살펴보면… 교정합니다」 같은 모델 해설문을 통째로 점역해
#   내보낸다(val 생물 p035·p061, 그 쪽 CER 50.40→59.75%). 대표가 정한 수정 로드 최우선
#   등급이 환각·프롬프트 문구 유출이다(2026-09-07). 그 대가로 얻는 칸수는 142만 자리 중
#   23자리(0.0016%)뿐이고, 그 23자리는 자모 오독 14 · 한글 음절 띄어쓰기 8 로 규칙이
#   못 닿고 코퍼스 빈도 말고는 근거가 없는 자리다(넣으면 과적합).
#   MinerU 토큰 칸(부호·도·낱자 곱·집합 묶음표)은 규정으로 채웠다(#764 · PR #765).
#   뿌리는 따로 남는다 — `transform` 이 폴백 응답에 안 걸리는 구멍(#768).
_PART_LLM_DEFAULT = {"태깅": "0", "표": "0", "수식": "0"}
