...
# 원본 자간 조판은 표지 글자 사이를 벌린다(`<보 기>`). 「점자 도서 제작 지침」[예 1-11]
# (재추출 423~429행)가 바로 그 꼴을 글상자 제목으로 싣는다 — 예외가 아니라 흔한 조판이다.
# 글자 사이 공백을 허용하지 않으면 그 상자는 태깅 LLM 호출조차 못 받아(실측: `외부LLM 0회`)
# 테두리가 한 칸도 안 나간다(#732 B).
_TAG_LABEL_WORDS = ("보기", "자료", "예시")
...
- 글상자: 대상 텍스트가 자료/보기 표지 줄로 '시작'하면 그 줄을 <!상자>표지<!/상자>로 바꾸고, 텍스트 맨 끝에 새 줄로 <!상자끝><!/상자끝>를 붙여 전체를 감싼다.
  ...
  ★ 원본 자간 조판으로 표지 글자 사이가 벌어진 꼴(예 <보 기>)도 같은 표지다. 글자·띄어쓰기는 바꾸지 말고 태그만 감싼다.
...
def _validate_tagging(original: str, tagged: str) -> bool:
    """LLM 태깅 결과 안전 검증: 알려진 태그만·테두리 짝·내용 보존·비어있지 않음."""
    if not tagged.strip():
        return False
    for _slash, name in _TAG_TOKEN_RE.findall(tagged):
        if name not in _KNOWN_TAGS:
            return False
    if not _borders_balanced(tagged):
        return False
