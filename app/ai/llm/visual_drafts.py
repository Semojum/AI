...
def omission_draft(label: str) -> Draft:
    ...
    ★ 이 주석은 **점수를 내주고 사는 것**이다 — 빼지 마라(2026-08-09 대표 결정).
      ...
      ★ 2026-08-10 대표 판정(원장 C-27) — **정답이 규정에서 벗어난 쪽이다.**
        정답 도서는 그림에 점역자주를 대부분 안 단다(10쪽 표본: 시각 든 6쪽 중 5쪽이 0회,
        '생략' 표기도 0회). 그건 **정답이 부족한 것**이지 우리가 과한 게 아니다.
        규정 우선 원칙대로 설명을 단다. **정답을 따라가려고 설명을 줄이지 마라.**
        (정답이 안 다는 이유는 그림 근처 본문이 이미 설명해서로 보이나 미검증 — C-27 참조.)
    """
