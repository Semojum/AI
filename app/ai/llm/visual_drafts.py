...
def _outline_text_indents(
    ...
    if _WRAP_STYLE == "box":
        ...
        # ★ 2026-09-10 원장 C-D4 재판정 채택 — **설명까지 주표 안**이다(만화와 같은 꼴).
        #   도서지침 3장 2절 4)에는 꼴을 정한 조항이 **둘**이고, 갈림선은 "그 글을 누가
        #   썼느냐" 다. 2026-09-08 에는 앞의 것만 읽고 정했다.
        #     · 4)(1)(2) L2367-2369·L2383-2385 — 제목 그대로 "**원본의 시각 자료에 있는
        #       내용 점역**". 주표 안에 유형만 적고 "쌍점 \"1과 **원본 자료의 내용**을 이어
        #       적는다" → `,'그림,'"1 내용`. [예 3-19]·[예 3-20] 원본 BRF 가 그 꼴이다.
        #     · 4)(3)① L2400-2401 — "**설명이 불충분하거나 원본 자료에 나와 있지 않은
        #       경우**  … 점역자 주표 ,' ,' 안에 시각 자료 유형, 쌍점 \"1, **설명할 내용**을
        #       적는다" → `,'그림"1 설명…,'`. [예 3-24]·[예 3-25]·[예 3-27]·자료지침 [예 6-1]
        #       원본 BRF 는 **항목 줄까지 한 주표 안**에 두고 덩이 끝에서 닫는다.
        #   우리 초안은 캡셔너가 그림을 보고 **지어 쓴 설명**이라 4)(3) 쪽이다. 원본에 인쇄된
        #   글을 옮겨 적는 갈래가 아니다 — 그러므로 조항을 잘못 골랐던 것이지 규정이
        #   관행에 진 것이 아니다.
        #   gold 도 같은 방향이다(원본 BRF 전수 `corpus/pages/braille` 18,892쪽 · 유형 낱말을
        #   닻으로 센 시각 머리줄 4,456건): **주 안 4,365(98.0%) · 주 밖 91(2.0%)**.
        #   그중 92.2% 는 머리줄이 아니라 **덩이 끝**에서 닫는다(전사 항목까지 주 안).
        #   재현: `temp/n10/r35-tnwrap/gold_census.py` · `span.py`.
        #   ⚠ **`diagram_opt` 골격 8종은 그대로 주 밖이다** — 그쪽은 원본 도형의 구조를
        #     옮겨 적는 4)(1)(2) 갈래이고 규정 예시도 밖이다(예3-46 조직도·3-47 연표·
        #     3-50/3-51 순서도). 두 꼴이 남는 것은 어긋남이 아니라 **조항이 둘이기 때문**이다.
        # ★ 만화는 종전부터 이 꼴이었다(원장 C-D6) — 이제 예외가 아니라 4)(3) 갈래의
        #   한 사례로 설명된다. 자료지침 예5-4·5-5 · gold 만화 98/98 이 쌍점·주 안이다.
        #   ⚠ 되돌리지 말 것 — 조문 한 줄만 읽고 쌍점을 뺐다가 두 번 되돌렸다.
        head_line = f"<!{_TAGS.TN}>{_oneline(head)}<!/{_TAGS.TN}>"
    ...
    if _WRAP_STYLE == "box":
        ...
        # ★ 2026-09-10 원장 C-D4 재판정 — **머리줄부터 전사 항목까지가 한 주표 안**이다.
        #   4)(3)① L2400-2401 이 "점역자 주표 안에 시각 자료 유형, 쌍점, **설명할 내용**을
        #   적는다" 이고, 그 조항의 예시 BRF(예3-24·3-25·3-27·자료지침 예6-1)가 항목 줄을
        #   전부 품은 뒤 덩이 끝에서 닫는다. gold 도 주-안 4,344건 중 92.2% 가 머리줄이
        #   아닌 뒷줄에서 닫는다(`temp/n10/r35-tnwrap/span.py`).
        #   ⚠ 만화는 여기서 제외한다 — **인물 대사는 주 밖**이라고 조항이 따로 정한다
        #     (자료지침 §5.3.3(2)(3)(5) L2827 "인물명 … 점역자 주표는 사용하지 않는다").
        #     만화는 종전대로 조항이 있는 자리(장면 표시 · 조립기가 감싸 보낸 행동·상황)만
        #     주 안이다. 넓혔다가 예3-54 의 `……` 까지 끌려 들어간 전례가 있다(C-D5).
        if kind != "만화":
            for k in range(tn_from, len(lines)):
                inside[k] = True
        # 아래 두 자리는 만화 몫이다 —
