...
# 머리줄 — `만화:` 말고 **유형 제시어**로 열어도 머리줄이다(§5.3.1(1) "'만화'를 …
# 5칸에 표기하고, 한 칸 띈 후 만화 제목을 적는다").
# ⚠ 이걸 안 보면 `그림: 인생 시기별 모습 다섯 칸` 이 아래 `_SAY_RE` 에 걸려
#   **인물 '그림'의 대사**로 실린다 — §5.3.3(5)가 인물명으로 삼으라는 것은 인물이
#   지닌 특징이지 유형 제시어가 아니다(실측: 테스트_이미지 5쪽, 캡션이 만화 형식을
#   안 지키고 `그림:` 으로 열었다).
_HEAD_RE = re.compile(r"^(?:만화|그림|사진|삽화)\s*[:：]\s*")
