...
_COMMON = (
    ...
    "- 자료에서 **확인한 사실**만 적되 (규정 7), 그중 **핵심에 초점**을 맞춥니다 (규정 2).\n"
    "  옷차림·자세·표정·색처럼 핵심과 상관없는 사실은 뺍니다. 빼는 것과 지어내는 것은\n"
    "  다릅니다 — 뺀 자리를 그림에 없는 뜻·기능·원인·평가로 메우지 마세요.\n"
    "  예: '현재: 학교에 다니고 있다'(O) / '학교 생활의 즐거움을 나타낸다'(X)\n"
    ...
    "  ② 칸·낱그림이 둘 이상으로 나뉘어 있으면 → 칸마다 한 줄.\n"
    "  ③ 화살표 사슬이 둘 이상이면 → 사슬마다 한 줄.\n"
...
_PROMPTS = {
    ...
    "image": _COMMON + (
        ...
        "  한 줄**, 한 그림 안에 이름표만 여럿이면 **한 줄에 쉼표로 이어** 나열하세요.\n"
        "  항목마다 설명을 붙이지 마세요.\n"
    ...
    "diagram": _COMMON + (
        ...
        "- 첫 줄에는 자료 전체가 무엇을 나타내는지를 **그림에 글자로 적혀 있을 때만** 쓰고,\n"
        "  적혀 있지 않으면 첫 줄을 두지 말고 바로 항목으로 들어가세요.\n"
