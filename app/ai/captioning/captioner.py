...
# 캡션 출력 상한. 위 acquire_sync 주석 참조.
_CAP_MAX_TOKENS = int(os.environ.get("CAPTION_MAX_TOKENS", "1500"))
...
_PROMPTS = {
    ...
    "diagram": _COMMON + (
        ...
        # ★ 2026-09-02 원장 C-94. 정답 도서 1,166개 설명 실측에서 나온 셋이다.
        #   ⓐ 판면 위치어 — 정답은 '왼쪽/오른쪽/가운데'를 **0번** 쓴다. 우리는 29번 썼다.
        #   ⓑ 범례 — 정답은 기호 풀이를 따로 두지 않고 항목을 뜻으로 곧장 부른다.
        #      '색칠' 0번·'동그라미' 0번인데 우리는 25번·29번 썼다.
        #   ⓒ 번호 — 정답은 없는 번호를 새로 매기지 않는다. 원본에 찍힌 ①·(가)를 그대로
        #      쓰는 것이 37%이고 `1. 2. 3.` 을 스스로 붙인 것은 1%다.
        #   (흐름도 골격의 규정 번호는 diagram_opt 가 따로 붙인다 — 여기와 충돌하지 않는다.)
        "- **판면 위치로 부르지 마세요.** '왼쪽:'·'오른쪽:'·'가운데:' 대신 그 부분의 **이름**으로\n"
        "  가리킵니다. 예) '왼쪽: 감수 1분열에서…'(X) / '감수 1분열에서…'(O)\n"
        "- **범례를 따로 두지 마세요.** '네모 정상 남자, 동그라미 여자' 를 앞에 늘어놓고 뒤에서\n"
        "  도형으로 부르지 말고, 처음부터 뜻으로 부릅니다. 예) '1(네모)'(X) / '1 정상 남자'(O)\n"
        "  그림꼴(빈 원·색칠·빗금·2쌍 모양)은 그것이 곧 값일 때만 적습니다.\n"
        "- **없는 번호를 새로 매기지 마세요.** 원본에 ①·(가)·1) 이 찍혀 있으면 그것을 그대로\n"
        "  쓰고, 아무 표시도 없으면 번호 없이 줄만 나눕니다.\n"
        "- 그림 속 기호를 뜻으로 푸는 자리는 **'A는 …, B는 …이다'** 한 문장으로 잇습니다.\n"
        "- 한 그림이 (가)·(나) 또는 [경우 1]·[경우 2] 로 나뉘면 **패널 이름을 줄머리에 두고**\n"
        "  패널마다 줄을 갈라 적으세요. 패널끼리 형식이 달라도 됩니다.\n"
        ...
        "- **가계도**는 역할·표현형·세대 관계를 화살표로 압축하고 **범례는 처음 한 번만**\n"
        "  적으세요. '부부 관계이다'·'자녀이다' 를 문장으로 풀지 마세요.\n"
        "  틀) `범례: 네모 남자, 동그라미 여자, 색칠 유전병` 뒤에\n"
        "      `아버지 정상 → 아들 정상 · 딸 유전병`"
    ),
    ...
    "chart": _COMMON + (
        ...
        # ★ 2026-09-02 원장 C-94. 정답 1,166개 실측에서 76건(6.5%)이 축 머리말로 연다.
        #   위 '눈금 전부 나열 금지'와 어긋나지 않게, 축 **이름과 단위**까지만 적게 한다.
        "- **축 머리말로 여세요** — `가로축은 A, 세로축은 B를 나타낸 그래프이다.` 를 먼저 쓰고\n"
        "  그 다음에 값을 적습니다. 단위가 적혀 있으면 축 이름에 괄호로 붙입니다\n"
        "  (`세로축(만 톤)`). 눈금 숫자를 처음부터 끝까지 늘어놓지는 마세요.\n"
        "- 출처가 인쇄돼 있으면 맨 끝에 `- 세계은행, 2016 -` 처럼 붙이세요.\n"
    ),
}
...
def _caption_anthropic(b64: str, mime: str, prompt: str) -> str:
    ...
    # ★ 상한 1,500 (2026-09-02). 500 으로는 복잡한 도표가 잘렸다 — 감수분열 흐름도가
    #   23번 항목 중간에서 끊겼고, 캡션 113건 중 13건이 400자를 넘어 상한에 닿아 있었다.
    #   사고는 꺼 두었으므로 상한은 본문 몫이고, 실제로 쓴 만큼만 값을 치른다.
    #   캐시 키는 이미지·백엔드·모델·프롬프트로 만들어 상한이 안 들어가므로 캐시는 안 깨진다.
    llm_limiter().acquire_sync(estimate_tokens(prompt, len(b64) * 3 // 4), _CAP_MAX_TOKENS)
