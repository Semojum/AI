...
_COMMON = (
    ...
    # ★ 2026-09-09 — **세부 줄**(들여쓰기 두 칸). 대표 결재 "간추린 설명 / 설명 /
    #   자세한 설명 으로 길이 적절히 조절해서 나오도록".
    #   초안은 **덜기만** 한다 — [12] 자세한 설명 = 재료 전부 · [2] 설명 = 들여쓰기 0 줄 ·
    #   [11] 간추린 설명 = 머리줄. 그래서 초안이 지어낼 자리는 구조적으로 없고, **재료를
    #   더 뽑는 이 한 자리**만 위험하다. 그 위험은 #665 하네스로 따로 잰다.
    #   조항: 「점자 자료 제작 지침」 §6.1.4(4) L3011-3012 "단계적 접근: 시각 자료의 전체
    #   윤곽을 포괄적으로 설명한 다음 **부분을 나누어 단계적으로** 설명한다".
    #   ⚠ 이 자리에서 실패한 전례가 둘이다. **둘 다 '줄 수 할당'이 원인이었다** —
    #     1차안 "항목이 다섯이면 다섯 줄"(요소오류 23.5%→28.6%, 지어냄 12→30줄) ·
    #     4차안 "부분마다 이름표가 붙어 있으면"(초안당 오독 8~9/60 → 14~16/60).
    #     모델이 **줄을 채우려고** 항목을 만들었다. 그래서 이 규칙은 할당을 안 준다:
    #     ① 없으면 한 줄도 안 쓴다고 못 박고 ② 그 부분에 **붙어 있는 값**으로 범위를
    #     좁히고 ③ 부분마다 붙일 필요 없다고 밝힌다. 관측만 늘리는 것이지 해석이 아니다.
    "- **세부 줄 — 두 칸 들여쓰기.** 한 줄(부분)에 적은 그것에 **더 읽히는 값**이 있으면,\n"
    "  그 줄 **바로 아래**에 두 칸을 들여 한 줄씩 적으세요. 이름표·수치·단위·개수·차례·\n"
    "  방향·모양이 그 값입니다. 전체 윤곽 → 부분 → 그 부분의 세부 순서가 됩니다.\n"
    "  예) 카드 1: 해파리 쏘임 사고 발생 건수 막대그래프이다.\n"
    "        2023년: 753건\n"
    "        2024년(1~9월): 4,224건\n"
    "  · **부분 줄은 그대로 두고 덧붙이기만 합니다.** 부분 줄에 이미 적은 것을 들여쓴 줄에\n"
    "    다시 쓰지 마세요.\n"
    "  · **그림에서 안 읽히면 안 씁니다 — 한 줄도 없어도 됩니다.** 들여쓴 줄을 채우려고\n"
    "    지어내는 것이 빠뜨리는 것보다 나쁩니다. 아는 지식·짐작은 한 글자도 쓰지 마세요.\n"
    "  · **맨 앞 줄과 부분 줄은 들여쓰지 마세요.** 들여쓰기는 세부 줄에만 씁니다.\n"
...
_PROMPTS = {
    ...
    "diagram": _COMMON + (
        ...
        # ★ 2026-09-10(#796) — 종전에는 "층이 셋 이상일 때만 번호를 쓰고, 층이 둘이면
        #   '윗 항목' 한 줄·'아래 항목들' 한 줄로 끝내라"였다. 그 한 줄이 §6.6.1(3)① 을
        #   못 서게 했다 — 조항은 **2단계도 상위 5칸·하위 3칸**으로 가르라고 한다
        #   (「점자 자료 제작 지침」 L3532). 캡션이 두 층을 한 줄씩으로 눌러 오면 하류
        #   (`diagram_structure.caption_outline`)가 전건 level 0 을 주고 8종이 전부
        #   같은 칸에 선다. 실측(캡션 캐시 3,242건): 골격이 선 699건 중 위계 2단계
        #   이상은 106건(15.2%)뿐이고 개념도는 408건 중 33건(8.1%)이었다.
        #   ⚠ 층을 **만들어 내라는 말이 아니다**. 그림에 층이 보일 때만 들여쓴다 —
        #     '줄을 채우려고 항목을 지어내는' 실패(1차안·4차안)를 되풀이하지 않도록
        #     할당을 안 주고 "안 보이면 들여쓰지 마세요"를 같은 자리에 박는다.
        "  ★ **층은 언제나 두 칸 들여쓰기로도 함께 나타내세요.** 번호를 쓰든 안 쓰든,\n"
        "    한 층 내려갈 때마다 그 줄을 **두 칸 더** 들여씁니다. 최상위 항목은 안 들여씁니다.\n"
        "  ★ **층이 둘뿐이어도 위계를 지우지 마세요.** 이때는 번호 없이 들여쓰기만으로 적되,\n"
        "    윗 항목 한 줄 · 아래 항목들 한 줄로 뭉뚱그리지 말고 **한 줄에 하나씩** 적으세요.\n"
        "    ⚠ **첫 줄(윤곽)은 항목이 아닙니다.** 최상위 항목도 첫 줄에 적지 말고 **그다음 줄**에\n"
        "      적으세요. 첫 줄은 이 도식이 무엇을 나타내는지의 한 문장입니다.\n"
        "    ⚠ 같은 항목을 **두 번 적지 마세요.** 한 항목은 자기 층에 한 번만 섭니다.\n"
        "    **층이 그림에 안 보이면 들여쓰지 마세요** — 나란히 놓인 것을 층으로 만들지 않습니다.\n"
        "- **화살표 →는 순서도·흐름도처럼 순서가 뜻인 도식에만** 쓰세요.\n"
        # ★ 2026-09-10(#796) — §6.6.2(4)⑤⑥(L3572-3574) "분기점에서 선택사항이 있는 경우
        #   … 3칸에 3o을 적고, 한 칸 띄어 선택사항, 그 후 3o과 목적지". 정답 예6-19 가
        #   그 꼴이다. 캡션이 갈림을 상자 줄 안에 풀어 넣어 오면 그 3칸 줄이 못 선다.
        "- **갈림(예·아니요 따위)이 있는 흐름도**는 갈림마다 줄을 바꿔 **두 칸 들여** 적으세요.\n"
        "  틀) 상자 줄 아래에 '  예 → 가는 곳' · '  아니요 → 가는 곳'. 갈림이 없으면 안 씁니다.\n"
        ...
        # ★ 2026-09-10(#796) — §6.6.4(2)②(L3663) "가장 처음 선조는 1칸에 적고, 후손
        #   단계에 따라 하위 수준으로 갈수록 두 칸씩 들여쓰기 한다". 세대를 가르는 자가
        #   캡션에 있어야 조립기가 칸을 세운다(`diagram_structure._generation_levels`).
        #   줄머리 세대 표지가 두 줄 미만이면 하류가 손대지 않으므로 폴백은 그대로다.
        "  세대가 갈리면 **줄머리에 '1세대'·'2세대'** 를 붙여 세대마다 줄을 나누세요.\n"
        "  그림에 세대 표시가 없으면 **가로줄로 나뉜 층을 위에서부터** 세대로 셉니다.\n"
...
# ★ 2026-09-10(#796) — **가름표를 요구한다.** 종전 정규식은 괄호도 쌍점도 없는 맨
#   '상황' 을 줄머리에서 떼어, 도표 캡션의 **낱말 하나를 통째로 먹었다** —
#   실물: 조직도 크롭 `  상황 맥락` → `맥락` (앞 빈칸까지 함께 사라져 위계도 무너졌다.
#   3회 재캡셔닝 중 2회 재현). `(?![가-힣])` 가드는 '상황실'만 막고 '상황 맥락' 은 못 막는다.
#   주석이 말하는 말머리 두 꼴은 **둘 다 가름표를 달고 있다** — `(상황)` 과 `상황:`.
#   가름표 없는 맨 낱말은 말머리가 아니라 본문이다.
# ★ 줄머리 빈칸은 group(1)로 되돌려 놓는다 — 들여쓰기가 §6.6 위계를 지는 자리다.
_SITUATION_HEAD_RE = re.compile(
    r"(?m)(^[ \t]*|^\s*\S{1,4}\s*[:：]\s*)"   # 줄머리(들여쓰기 보존) 또는 '만화:' 뒤
    r"(?:[(（\[【]\s*상황\s*[)）\]】]\s*[:：]?"  # (상황) — 괄호가 가름표라 쌍점은 선택
    r"|상황(?![가-힣])\s*[:：])\s*")            # 상황: — 괄호가 없으면 쌍점을 요구한다
