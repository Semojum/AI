...
# ★ 2026-08-15 A/B 실험군 — • 글머리를 규정형 ⠸⠲로 바꿔 본다. 판정은 재점역 A/B가 한다.
#
# 여기가 **진짜 생성처**다. `layout_braille._HIDDEN_TO_BULLET`에도 `"⠐⠆"` 분기가 있지만
# 그건 **죽은 분기**다 — 거기로 오려면 •가 가운뎃점 ⠐⠆로 남아 있어야 하는데, 이 표가
# 점역 단계에서 이미 ⠔⠔로 바꿔 버린다(실측: 우리 출력의 줄머리 ⠔⠔ 443개가 전부 이 경로,
# ⠐⠆는 18개뿐이고 그나마 줄머리가 아니다). 처음에 그 죽은 분기를 고쳐 A/B가 0쪽 변화로
# 나왔다.
#
# 규정 제72항은 • 글머리를 ⠸⠲(_4)로 정한다. 종전 관행 ⠔⠔(99)의 근거는 구판 1,131p
# ASCII 원문 스캔의 "_4 0회 vs 줄머리 99가 2,343줄"이었는데 2027은 정반대다:
#     구판 1,131p   99 = 2,343 · _4 =     0
#     dev-2027      99 =     0 · _4 = 1,555
#     val-2027      99 =     0 · _4 = 1,602
# 대조군 _0(○)이 dev 211·val 193으로 같은 스캔에 잡히니 측정 실패가 아니다. ASCII 원문을
# 세므로 점자 셀이 한글 자모와 겹치는 오염도 없다.
#
# 앞서 같은 뿌리 넷이 뒤집혔다(원장 R-13·R-14·R-15·R-16). 이 항목은 수식 축이 아니라
# 텍스트·레이아웃 축이라 **val에서도 판정된다**(용례 dev 1,555 · val 1,602).
# 기각되면 ⠔⠔로 되돌린다.
SYMBOL_TABLE["•"] = "⠸⠲"
