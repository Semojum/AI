...
# 그리스 소문자 접두 — **규정형 ⠨**(2026-07-29 대표 결정). 2026-08-27 교차 검증으로 뒷받침됨.
#
# | 칸 | 값 |
# |---|---|
# | 근거 범위 | 종전 근거는 **frozen 수학2 한 과목** (val 263:0 · dev 24:0, `output_수학2_page028.brl`) |
# | 적용 범위 | 모든 과목의 그리스 소문자 |
# | 교차 검증 | **dev-2027 재측정 — 관행 ⠈ 0 : 규정 ⠨ 344** (생명 0:53 · 수학1 0:291) |
# | 어느 코퍼스 | 근거 `frozen` / 교차 `dev-2027` |
#
# **dev-2027 에는 수학2 가 아예 없다.** 판본이 다른 관행을 2027 에 먹이고 있었다.
# 글자별로도 한쪽이다 — α 0:27 · β 0:26 · θ 0:198 · π 0:51.
# ⚠ 측정법 주의: `⠈X`·`⠨X` 2셀은 **한글 음절과 겹친다**(원장 C-03 의 ⠣ 282,263회 오측과 같은 함정).
#   그냥 세면 국어 교재에서 '그리스 소문자 8,037회' 같은 값이 나온다. 위 수치는
#   **묵자에 그 글자가 있는 쪽 vs 없는 쪽**으로 배경을 뺀 값이다.
# 규모는 크지 않다 — dev-2027 900쪽 묵자에 그리스 소문자 89건 / 23쪽.
...
def _load_flat_table() -> dict[str, str]:
    ...
    #   관행형이던 근거(gold 판정가능 265건 중 ⠈ 263)는 **frozen 수학2 한 과목**의 것이다.
    #   ★ 2026-08-27 dev-2027 재측정으로 그 결정이 옳았음이 확인됐다 — **⠈ 0 : ⠨ 344**.
    #     dev-2027 에는 수학2 가 아예 없다. 판본이 다른 관행이었다(파일 상단 표 참조).
...
# ○□△ 줄머리 = 제72항 글머리 기호(꼬리 ⠇ 없음), 줄 안 = 제49·57항 숨김표(꼬리 ⠇ 있음).
# SYMBOL_TABLE은 기호 하나에 값 하나라 숨김표 쪽만 담는다(C-19). 줄머리 정정은 이 표로 한다.
#   근거 「한국 점자 규정」 braille-source/text/한국 점자 규정_재추출.txt
#     행 2879~2888  제72항 글머리 기호  ○=_0 □=_7 △=_+ •=_4
#     행 2890~2901  제72항 예시 "□ 2021 세계한국어한마당 / ○ (기간/방식)…" — 줄머리에 꼬리 없음
#     행 2206~2218  제49항 숨김표      ○=_0l △=_+l □=_7l (×=_xl)
#     행 2493~2502  제57항 반복 숨김표 _++l 등 — 반복형은 이 표에 안 걸린다(붙어 나오는 △△고 보호)
# 원장 C-07·B-03. 정본은 여기다 — layout_braille이 이 표를 가져다 쓴다.
HIDDEN_TO_BULLET: dict[str, str] = {
    "⠸⠴⠇": "⠸⠴",   # ○
    "⠸⠶⠇": "⠸⠶",   # □
    "⠸⠬⠇": "⠸⠬",   # △
}
