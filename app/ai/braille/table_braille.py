...
def _should_transpose(rows: list[list[str]], n_cols: int) -> bool:
    ...
    실효 규칙은 "원본이 행 단위로 안 들어가고 + 열 수 > 행 수"다.

    ⚠ 2026-07-27 정정 — 여기 있던 후보 규칙 검정표(①먼저→C>R 87.4% · C>R 88.2% ·
      ①먼저→C>R and T폭<폭 86.6% · T폭<폭 84.9% · 전치조건 73.1% · "전치 안 함" 고정
      71.4%)는 **근거를 잃었다. 인용하지 말 것.** 그 수치는 전부 2026-07-21 라운드가
      "방향이 확증된 119요소"라 부른 라벨 위에서 계산됐는데, 그 라벨은 **격자에서 이웃한 두
      셀이 gold에 붙어 나오는가**(셀 인접)만 셌다. 이 신호는 한 줄에 셀을 연달아 적는
      배치(rowwise·colwise)에서만 켜지고, 머리 셀과 구분자가 사이에 끼는 전개형
      (col_unfold·row_unfold)에서는 꺼진다 — 그런데 gold가 가장 많이 쓰는 배치가 그
      전개형이다. 2026-07-27 재라벨(s4 세트 149건, 배치 4안 순서 대조 + 자기정합 95.2%)과
      겹치는 61건에서 구 라벨의 일치율은 **54.1%**(동전 던지기)였고, 불일치 3건을 gold
      원문으로 눈검사하니 **3건 모두 구 라벨이 오답**이었다(사회문화 p087·p084·p114).
      재현: V2/temp/s4_collect.py · s4_label.py · s4_build.py → s4_table_axis_set.json.

    ★ 그래도 규칙 자체(C > R)는 **유지한다.** 근거가 두 계통이었고 하나는 살아 있다.
      · 살아 있는 근거 ①: 전 코퍼스 재점역 A/B(아래 ⚠ 항목의 dev/val 편집셀 수치)는
        라벨과 무관하게 측정된 것이다.
      · 살아 있는 근거 ②: 생물 p119(3×6 → 6×3) — gold가 열 인접쌍 123/123(100%)으로
        전치본을 적었고, 원본 행 폭 52칸(>32)이 전치 후 28칸(≤32)으로 줄어 §3.1.1(1)①을
        만족한다. 새 s4 라벨도 이 표를 col로 확정해 같은 결론이다.
      · 새 세트에서 이 경로(전치+rowwise)의 축 정확도는 4/5(80%)로 여전히 상수보다 낫다.
      다시 손볼 거면 s4 세트로 재검정하고 채점기 소유자와 함께 전 코퍼스 A/B를 붙일 것.
...
def _render_unfold(corrected_text: str) -> list[str]:
    ...
    블록 형태(정답 도서 = 수능특강 점역본 실측):
        수급 분류  60—64세      ← 모서리 라벨 + 블록 머리
        연금 수급자  68.3        ← 딸린 머리 + 값
        기초 수급자  3.2
    즉 개체마다 머리 줄을 세우고 그 아래 "상대축 머리  값"을 한 줄씩 적는다. 32칸 안에
    ...
    ★ 개체축 = **행**이다(2026-07-27 정정). 그전에는 ①·(2)가 둘 다 불발한 표를 원본
      격자 그대로 전개해 **열마다** 블록을 세웠다(이하 '열 폴백').
      · 규정: §3.3.1은 풀어 적는 표를 "(3) 행 제목은 3칸에 적고, 열 항목은 열 제목 순서에
        따라 두 칸씩 띄어 적는다 / (4) 줄바꿈: 행 제목 단위로 줄을 바꿔 적는다"로 규정한다.
        예 3-7(주요 국가들의 고령화 속도 비교)도 열 제목을 맨 위에 한 번만 적고 그 아래를
        프랑스·미국·독일… 로 **행마다** 끊는다. 열마다 블록을 세우라는 조항은 없다.
      · gold 실측(s4 판정 세트 149건, dev+val, 재현 스크립트 V2/temp/s4_*.py):
        열 폴백으로 떨어지던 102건의 정답 개체축은 행 67 : 열 35로 **행이 다수**다.
        그 구간 축 정확도 34.3%(35/102) → 65.7%(67/102). 전체 149건 45.0% → 66.4%로
        "무조건 행" 상수(64.4%)도 넘는다(①·(2) 경로가 93%·80%로 정보를 더하기 때문).
      · 눈검사: 사회문화 p087 gold는 `피라미드형 계층 구조`(=행)를 5칸 블록 머리로 세우고
        그 아래 3칸에 `의미⠐⠂ …` `특징⠐⠂ …`를 적는다 — 정확히 행 개체 블록이다.
      ⚠ 여기서 잰 것은 **개체축 판정 정확도뿐**이고 CER 이득이 아니다. 셀 편집 증감은
        채점기로 따로 재야 한다(이 라운드에서는 측정하지 않았다).
      ⚠ 남은 부정합 1건: (2)가 발동해 전치했지만 전치본도 한 줄에 안 들어가 여기로 오는
        경로(s4 기준 28건)는 이제 폴백과 **같은 배치**(행 개체)를 내면서 점역자 주만 더
        붙는다. 그 주를 뗄지는 별도 A/B 대상 — 이번 변경에 묶지 않았다.
    """
    ...
    # ★ 풀어 적는 표의 개체는 **행**이다 — §3.3.1(4) "줄바꿈: 행 제목 단위로 줄을 바꿔
    #   적는다"(§3.3.1(3)도 '행 제목은 3칸, 열 항목은 열 제목 순서에 따라'로 행이 바깥
    #   단위임을 전제한다. 예 3-7도 열 제목을 맨 위 한 번만 적고 그 아래를 행마다 끊는다).
    #   아래 전개 코드는 "격자의 열마다 블록"을 세우므로, 행을 개체로 펴려면 **전치본**을
    #   넣어야 한다. 그래서 두 갈래 모두 전치본으로 들어간다(실측 근거는 함수 독스트링).
    grid, glen, gnc = _transpose_rows(rows, orig_len)
    ...
    if _should_transpose(rows, n_cols):          # §3.1.1(2) 넓은 표 → 행↔열 교환
        ...
        if _rowwise_ok(grid):                    # 전치 후 한 행이 들어가면 행 단위
            return head + _render_rowwise(grid, glen)
    rows, orig_len, n_cols = grid, glen, gnc
