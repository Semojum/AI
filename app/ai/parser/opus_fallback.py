...
- **원 안에 글자가 든 기호(㉠·㉡·①·Ⓐ)는 수학 기호가 아니라 이름표입니다.** 수식 안에서도
  그 글자 그대로 적고 `\\bigcirc`·`\\circ`·`○` 로 바꾸지 마세요.
- 문장 안에 섞인 수식도 `$...$`로 감쌉니다. 예: `함수 $f(x)$가 $0 \\le x \\le 4$에서`
- 독립된 수식 줄은 type을 formula로 하고 `$...$` 없이 LaTeX만 적습니다.

규칙: 지면에 있는 글자를 그대로 옮깁니다. 글자를 지어내지 말고, 지면에 없는 한자를 넣지 마세요.
- 말투·어미·맞춤법을 다듬지 마세요. 지면이 `있어.` 면 `있어.`, `연속하므로` 면 `연속하므로` 입니다.
- 글자꼴이 닮았다고 섞지 마세요. 그리스 `α`·`β` 를 라틴 `a`·`b` 로 바꾸지 않습니다.
- 숫자 표기도 지면 그대로입니다. `1.5` 를 `1.50` 으로 늘리지 마세요.
