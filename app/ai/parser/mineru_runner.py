...
def _run_mineru(pdf_path: Path, out_dir: Path, page_idx: int, timeout: float | None = None) -> None:
    ...
    #     ─ 2026-09-15 · **vLLM 실측**(수학2 8쪽, 로그 타임스탬프에서 뽑은 추출 시간):
    #         · high  **중앙 14.0초 · 최대 25.0초**
    #         · medium **중앙 5.0초**
    #       옛 1.70배를 그대로 얹어도 25×1.70 = 42.5초로 **예산의 24%** 다.
    #       즉 "예산 초과" 라는 이 판단의 근거는 더는 서지 않는다. 기본값을 바꿀지는
    #       손해를 전수로 재고 정한다(#886 라운드).
    #   품질 대가는 같은 dev 28쪽 짝 비교로 −0.85p뿐이라던 것도 같은 시절 값이다.
    #   (2026-09-15 수학2 8쪽 실측으로는 high 가 formula 요소 16→48 · 기형 LaTeX 40→26 으로
    #    오히려 낫다. 표본이 "이미 잃은 쪽" 이라 기울었으니 전수 판정 전까지는 참고만.)
