...
# ── 줄바꿈 조각 잇기(C005) ───────────────────────────────────────────────────
# 좁은 글상자 안에서 줄을 바꾼 자리가 **PDF 텍스트 레이어의 블록 경계**로 굳어, 한 문장이
# 요소 여러 개로 쪼개져 나온다("…연결되어 있습니" / "다. 시각, 청각 등의"). MinerU가
# 만든 것이 아니라 원본이 그렇고, 우리가 그 경계를 그대로 옮긴다. 쪼개진 채로 나가면
# 문단 구조·캡션 문맥·조판이 전부 그 경계를 진짜 경계로 오해한다.
#
...
#   ③ 이 시행·목록과 참-줄바꿈을 가르는 신호다. 시행·목록은 할 말이 끝나 줄이 짧다.
_JOIN_TYPES = {"text"}
_JOIN_LINE_GAP = 1.2        # 다음 줄까지 세로 간격(줄 높이 배수)
_JOIN_X_TOL = 1.5           # 왼쪽 끝 어긋남(줄 높이 배수)
_JOIN_RIGHT_SLACK = 1.2     # 오른쪽 끝에서 모자란 칸(줄 높이 배수) — 한글은 글자 단위로
                            # 줄을 바꾸므로 마지막 글자 하나만큼은 늘 남는다
_JOIN_HEIGHT_TOL = 0.6      # 글자 크기 어긋남(줄 높이 배수)
_JOIN_NEAR_LINES = 25       # 단 오른쪽 끝을 재려고 볼 위아래 줄 수
_JOIN_MIN_WIDTH = 6.0       # 앞 조각 최소 너비(줄 높이 배수) — 'EBS'·'방사관' 같은 라벨 배제
...
def _col_edge(lines: list[list[float]], bb: list[float]) -> tuple[float, float]:
    """같은 단 이웃 줄들의 (오른쪽 끝 최댓값, 그 끝에 닿는 줄의 비율).

    비율이 잇기 판정의 핵심이다. **넘쳐서 접힌 문단은 거의 모든 줄이 오른쪽 끝에 닿고**,
    시행·목록·낱말 나열은 할 말이 끝나는 데서 줄이 끊겨 끝이 들쭉날쭉하다. 최댓값만
    보면 시행 중 가장 긴 줄이 스스로 '단 끝'이 돼 버려 시가 통째로 이어진다
    (언어 p37·p39·p55·p56 실측).
    """
    line = bb[3] - bb[1]
    xs = [b[2] for b in lines
          if b != bb
          and b[2] > bb[0] and b[0] < bb[2]                    # 가로로 겹침
          and abs(b[1] - bb[1]) < _JOIN_NEAR_LINES * line]     # 세로로 가까움
    if not xs:
        return 0.0, 1.0
    # ★ 단 끝은 **최댓값**으로 잡는다. '가장 많은 줄이 공유하는 끝'을 쓰면 문단 몇 개를
    #   더 잇지만(시연문서 p1 이음 21→28) 가사·시조가 딸려 온다 — 정형시는 율격이 고정
    #   이라 행 길이가 고르고, 그래서 '고른 오른쪽 끝'이 문단과 구별이 안 된다
    #   (언어 234쪽 실측: 오탐 0건 → 20건). **잘못 잇는 쪽이 더 나쁘다**(pm 지시).
    right = max(max(xs), bb[2])
    flush = sum(1 for x in xs if right - x <= _JOIN_RIGHT_SLACK * line)
    return right, flush / len(xs)


def _line_seam(fitz_page: fitz.Page, bb: list[float], text: str) -> str:
    """이을 때 두 조각 사이에 넣을 것.

    한 줄이 낱말 한가운데서 잘렸으면 붙여야 하고("있습니"+"다."), 어절 끝에서 잘렸으면
    한 칸 띄어야 한다("통합한 뒤"+"감각"). 그 차이는 **원본 줄 끝의 공백**에만 남아 있다.

    ⚠ 교과서 PDF 다수는 공백 글리프가 아예 없고 커닝으로만 어절을 띄운다
      (`_native_text_spaced` 주석 참조). 그런 줄에서 끝 공백이 없는 것은 아무 뜻도 없다 —
      실제로 이 판정을 느슨하게 뒀다가 '유대 관계를'+'강화하였다.'를 붙여 버렸다.
      그래서 **셋을 다 확인했을 때만** 붙이거나 띄우고, 하나라도 어긋나면 개행을 남겨
      종전 경로(layout_braille._fold_full_lines)가 정하게 한다.
        ① 레이어에서 읽은 마지막 줄이 이 요소의 마지막 줄과 **같은 글자**인가
        ② 그 줄의 공백 수가 우리가 복원해 둔 어절 수만큼 되는가. 쉼표 뒤 공백만 있고
           어절 사이는 커닝인 지면이 있다("폐포, 혈액, 조직세포에서" — 실측). 개수를
           맞대면 정확히 갈린다
        ③ 그 줄이 공백으로 끝나는가 → 띄움, 아니면 → 붙임
    """
    w, h = fitz_page.rect.width, fitz_page.rect.height
    raw = fitz_page.get_textbox(fitz.Rect(bb[0] / 1000 * w, bb[1] / 1000 * h,
                                          bb[2] / 1000 * w, bb[3] / 1000 * h))
    if not raw.strip():
        return "\n"
    last = raw.rstrip("\n").split("\n")[-1]
    want = (text or "").rstrip().split("\n")[-1]
    if not want or "".join(last.split()) != "".join(want.split()):   # ① 같은 줄을 읽었나
        return "\n"
    if last.count(" ") < want.count(" "):                            # ② 커닝만 쓰는 줄
        return "\n"
    return " " if last.endswith((" ", "\u00a0")) else ""            # ③


def _join_wrapped_lines(elements: list[dict], fitz_page: fitz.Page) -> list[dict]:
    """줄바꿈으로 쪼개진 이웃 텍스트 요소를 하나로 잇는다. 위 주석의 판정 셋을 쓴다.

    ⚠ 이은 요소의 bbox 는 여러 줄의 합집합이라 높이가 줄 높이가 아니다. 다음 줄과 견줄
      때는 **마지막으로 이어붙인 줄의 bbox**(tail)를 써야 한다 — 합집합을 쓰면 두 줄째
      부터 글자 크기 판정에 걸려 사슬이 끊긴다(한 번 밟았다).
    """
    # ★ 단 오른쪽 끝은 **이어붙이기 전 줄 bbox**로 잰다. 이어붙이면 prev["bbox"]가 여러
    #   줄의 합집합으로 바뀌는데, 그게 elements 안의 같은 dict라 재는 쪽 시야가 같이
    #   망가진다(문단 중간에서 사슬이 끊겼다 — 한 번 밟았다).
    lines = [list(e["bbox"]) for e in elements
             if e.get("type") in _JOIN_TYPES and e.get("bbox")]
    out: list[dict] = []
    tail: list[float] | None = None       # out[-1] 의 마지막 줄 bbox
    tail_text = ""                        # 그 줄의 글(이어붙이기 전 것) — 이음매 판정용
    left_ok = False                       # out[-1] 이 왼쪽으로 쓸 수 있는가(한 줄이었거나 우리가 이은 것)
    for el in elements:
        if out and tail and left_ok and _can_join(lines, out[-1], tail, el):
            prev = out[-1]
            # ⚠ 이음매는 **마지막 줄의 글**로 판정한다. 이어붙인 prev["content"] 를 주면
            #   두 번째 이음부터 줄 대조가 어긋나 늘 개행으로 물러선다(한 번 밟았다).
            seam = _line_seam(fitz_page, tail, tail_text)
            prev["content"] = f"{prev['content']}{seam}{el['content']}"
            prev["bbox"] = [min(prev["bbox"][0], el["bbox"][0]),
                            min(prev["bbox"][1], el["bbox"][1]),
                            max(prev["bbox"][2], el["bbox"][2]),
                            max(prev["bbox"][3], el["bbox"][3])]
            if prev.get("bbox_px") and el.get("bbox_px"):
                prev["bbox_px"] = [min(prev["bbox_px"][0], el["bbox_px"][0]),
                                   min(prev["bbox_px"][1], el["bbox_px"][1]),
                                   max(prev["bbox_px"][2], el["bbox_px"][2]),
                                   max(prev["bbox_px"][3], el["bbox_px"][3])]
            tail, tail_text = list(el["bbox"]), el["content"]
            continue
        out.append(el)
        tail = list(el["bbox"]) if el.get("bbox") else None
...
def _can_join(lines: list[list[float]], a: dict, ab: list[float], b: dict) -> bool:
    ...
    if not at or not bt:
        return False
    # ★ 조각내진 것은 **인쇄 한 줄**짜리다. 여러 줄 블록은 MinerU 가 이미 문단으로 묶은
    ...
    if abs(ah - bh) > _JOIN_HEIGHT_TOL * line:
        return False
    if not (-0.5 * line <= bb[1] - ab[3] <= _JOIN_LINE_GAP * line):
        return False
    if abs(bb[0] - ab[0]) > _JOIN_X_TOL * line:
        return False
    if (ab[2] - ab[0]) < _JOIN_MIN_WIDTH * line:      # 서너 글자짜리는 단이 아니다(머리말·라벨)
        return False
