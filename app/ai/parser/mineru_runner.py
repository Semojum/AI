...
def _run_mineru(pdf_path: Path, out_dir: Path, page_idx: int, timeout: float | None = None) -> None:
    ...
    # ★ 기본값을 medium으로 둔다 — 종전 근거는 **180초 페이지 예산(C7)을 못 지킨다** 였다.
    #   ⚠ 그 근거는 **낡았다(2026-09-15 확인).** 아래 2026-07-29 수치는 **transformers 엔진
    #     시절 값**이고, 지금은 vLLM 이라 자릿수가 다르다. 지우지 않고 그대로 두되 표시한다.
    #     ─ 2026-07-29 · **transformers 엔진 시절 값**(시각요소 최다 10쪽 + 동시성 시험):
    ...
    #     ─ 2026-09-15 · **vLLM 실측**(#886. 수학2 147쪽에서 **무작위 20쪽**,
    #       `random.Random(886).sample`. 추출 상한 300초를 주고 시간만 쟀다 — 상한에
    #       걸린 쪽 0건이라 분포가 안 잘렸다. 첫 호출의 모델 적재는 표본 밖 쪽으로 뺐다):
    #         · high   중앙 **11.7초** · 최대 **19.5초** · p90 13.0초 · **60초 초과 0/20**
    #         · medium 중앙   4.7초  · 최대   9.4초  · p90  5.2초
    #       동시 배율도 같은 엔진에서 다시 쟀다 — **동시2 = 1.71배 · 동시3 = 2.15배**
    #       (옛 1.70배가 vLLM 에서도 그대로 나왔다. 이 값만은 엔진을 넘어 산다).
    #       제품은 `mineru_max_concurrent = 2` 다. 지금까지 본 가장 느린 쪽 25.0초
    #       (무작위 20쪽 최대 19.5 · 편향 8쪽 최대 25.0, 합쳐 28쪽)에 1.71 을 얹으면
    #       **42.8초 = 60초 추출 상한의 71% · 180초 페이지 예산의 24%.**
    #       ⚠ 여유가 medium 3.7배에서 **high 1.4배**로 준다. 60초는 성능 조절기가 아니라
    #         **비정상 탐지기**이고 그 눈금이 medium 기준(p99 36.6초)으로 매겨져 있다
    #         (`config.mineru_timeout_resolved`). high 로 바꾸려면 이 눈금도 같이 다시
    #         매겨야 한다 — 안 그러면 조금 느린 정상 쪽을 탐지기가 끊는다.
    #       ⚠ 꼬리는 얇다. 28쪽/147쪽이라 p95 위는 덜 봤다. "26초 넘는 쪽이 없다" 가
    #         아니라 "이 28쪽에는 없다" 다.
    #       확실한 것은 여기까지다 — **"예산 초과" 라는 옛 근거는 더는 서지 않는다.**
    #       품질이 그만한 값을 하는지는 **별개**이고 아직 안 정했다(#886 전량 A/B).
    #     ─ 종전에 여기 적혀 있던 "8쪽 중앙 14.0초" 는 무작위가 아니라 "조각을 많이 잃은
    #       쪽" 으로 고른 표본이었다. 위 무작위 20쪽으로 대체한다(방향은 같다 — 무거운
    #       쪽을 골랐으니 8쪽이 더 느리게 나온 것이 맞다).
    #   품질 대가는 같은 dev 28쪽 짝 비교로 −0.85p뿐이라던 것도 같은 시절 값이다.
    #   (2026-09-15 수학2 8쪽으로는 high 가 formula 요소 16→48 · 기형 LaTeX 40→26 으로
    #    오히려 낫다. 같은 기운 표본이니 전수 판정 전까지는 참고만.)
    # 목록 구조는 effort로 사지 말고 후처리(글머리 기호로 list_item 승격)로 회복할 것.
    # high로 올리려면 **원가를 재산정**한 뒤여야 한다. 같이 적혀 있던 "동시 처리를 1로
    #   낮추고" 는 예산 초과에서 나온 조건이라 vLLM 실측(위)에서는 안 선다 — 동시 2 에서
