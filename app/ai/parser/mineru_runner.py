...
def _run_mineru(pdf_path: Path, out_dir: Path, page_idx: int, timeout: float | None = None) -> None:
    ...
    #     ─ 2026-09-15 · **vLLM 관측**(수학2 **8쪽**, 로그 타임스탬프에서 뽑은 추출 시간):
    #         · high  중앙 14.0초 · 최대 25.0초
    #         · medium 중앙 5.0초
    #       ⚠ **이 8쪽을 수학2의 분포로 읽지 말 것.** 무작위가 아니라 "조각을 많이 잃은 쪽"
    #         으로 고른 표본이라 대표성이 없고, 8쪽으로는 오래 걸리는 꼬리가 안 담긴다.
    #         옛 1.70배를 얹은 42.5초(예산의 24%)도 이 8쪽에 한정된 환산이다.
    #       말할 수 있는 것은 여기까지다 — **"예산 초과" 라는 옛 근거가 더는 서지 않는다.**
    #       "그러니 high 를 써도 된다" 는 **아직 근거가 없다.** 전량 조건의 시간 분포는
    #       무작위 표본으로 따로 재는 중이다(#886).
    ...
    # high로 올리려면 **동시 처리를 1로 낮추고 원가를 재산정**한 뒤여야 한다 — 이 조건은
    #   엔진이 바뀌어도 그대로다. 위 수치는 기본값을 바꿀 근거가 아니라 옛 근거의 만료 표시다.
