...
def decode(braille: str, *, math: bool = False) -> str:
    ...
    braille = _mark_paren_pairs(_strip_hangul_indicator(
        _strip_bold_marks(_strip_typeform_marks(braille))))
...
# ★ **줄을 쪼개기 전에** 뗀다(_strip_typeform_marks). _COMBINED 에 넣으면 한글 경로
#   깊은 곳에서야 사라져 _english_line(제29항 [다만] · #482)이 이 줄을 영어로 못 읽는다.
_UEB_UNDERLINE = ("⠸⠂", "⠸⠄")


def _strip_typeform_marks(text: str) -> str:
    """UEB 밑줄 낱말표·종료표를 벗긴다. 묵자에 대응 문자가 없는 서식 표시다."""
    for _c in _UEB_UNDERLINE:
