...
# 로그의 밑이 숫자일 때(「수학 점자」 제46항 1호) — `⠸⠠` 뒤에 **수표 없이 내려 적는다**
# (규정 예시 `_,5#b` = ⠸⠠⠢⠼⠃ = log₅2). 내려 적은 숫자는 셀이 연산·괄호와 겹쳐서
# (⠢=+ · ⠦=여는 괄호 · ⠴=닫는 괄호) 홀로는 못 가른다 — **⠸⠠ 에 붙은 자리로 한정**해
# 표에 박는다. 긴 것부터 맞으므로 두 자리 밑(log₁₀·log₂₇·log₃₀·log₅₀)이 먼저 걸린다.
# 실측(전 코퍼스 18,892쪽): ⠸⠠ 486회·52쪽. 그중 441회가 내려 적기가 붙은 로그이고
# 두 자리 밑 14회·⠦(8) 2회는 재추출 묵자에 log_{10}5·log_{27}4·log_{30}2·log_{50}3·
# log_{8}2 로 **건수까지 일치**했다. 나머지 45회는 뒤에 내려 적기가 없다(대문자표
# `⠸⠠⠝`=log N 등) — 표에 안 걸리므로 종전대로 둔다.
# 밑을 `{}` 로 감싸는 건 진수가 바로 이어 붙기 때문이다(`log_26` 은 log₂6 과 log₂₆ 을
# 못 가른다). 재추출 묵자도 `\log_{2}` 로 적는다.
_LOG_LOW_DIGIT = dict(zip("⠂⠆⠒⠲⠢⠖⠶⠦⠔⠴", "1234567890"))
_MATH_REV_MULTI.update(
    {"⠸⠠" + a: f"log_{{{x}}}" for a, x in _LOG_LOW_DIGIT.items()}
)
_MATH_REV_MULTI.update(
    {"⠸⠠" + a + b: f"log_{{{x}{y}}}"
     for a, x in _LOG_LOW_DIGIT.items() for b, y in _LOG_LOW_DIGIT.items()}
)
# 밑이 문자일 때(제46항 2호) — `⠸⠰` 뒤 **한 칸이 밑**이다(규정 예시 `_;AN` = log_a n).
# 두 칸(`⠸⠰`)만 표에 있으면 밑 글자가 따로 풀려 **한글 약자로 샜다** — `⠸⠰⠁⠝` 이
# `log_그런데`(⠁=그런데)로 나갔다. 밑까지 한 덩어리로 잡아 로마자로 고정한다.
...
# 다만 **토큰 첫 칸이 홑 로마자이고 바로 뒤에 오는 자리**는 겹칠 것이 없다 — 한글은 낱자
# 하나로 낱말을 시작하지 않는다. 실측(표본 2,362쪽) 16건이 전부 수식이고 손해가 없다:
#   ⠭⠲⠲⠼⠚ 옥≥0 -> x≥0 · ⠍⠲⠲⠼⠉⠠⠴ 우..3) -> m≥3) · ⠝⠲⠲⠼⠓ 에..8 -> n≥8
# ⠨ 는 부정 부등호(≱ 제4항 3호)의 앞 셀이라 사이에 낄 수 있다.
_MATH_GE_HEAD_RE = re.compile(r"^[⠁⠃⠉⠙⠑⠋⠛⠓⠊⠚⠅⠇⠍⠝⠕⠏⠟⠗⠎⠞⠥⠧⠺⠭⠽⠵]⠨?⠲⠲")
...
# 가른다. symbol_table 은 정방향용이라 사각문자(㎝·㎏…) 쪽으로만 등록돼 있고, 역맵이
# 그걸 그대로 되돌려 `100cm이다` 가 `100㎝이다` 로 나갔다.
# ★ 어느 쪽으로 펼지는 실측으로 정한다:
#   · 규정 제69항 예문 자체가 `cm`·`kg`·`mm`·`in`·`cal`·`min` 을 **로마자**로 싣는다
#     (사각문자로 적은 자리는 ㎎·㎗·㎞·㎠ 뿐이다).
#   · 재추출 묵자 1,361쪽 전수: 로마자꼴 **114회** : 사각문자 **1회**(㎢).
#   · 그 셀이 실제로 있는 쪽만 좁혀도 묵자는 cm 19 · kg 5 · km 6 · mm 5 · mg 2 · mL 1,
#     **사각문자는 0회**다. ㎡ 자리의 묵자는 `m^{2}`(LaTeX)라 `m^2` 로 편다.
# ★ `⠴⠍⠍⠲` 는 표 자체가 틀렸다 — 규정 [붙임1]에서 그건 `mm` 이고 μm 은 그리스 문자
#   접두가 붙은 `⠴⠨⠍⠍⠲` 다(`1 μm는 1,000분의 1 mm이다.` -> `#a`0.mm4cz…#a`0mm4oi4`).
# ★ **값만 바꾼다 — 키(셀 시퀀스)는 그대로다.** 소비 길이가 안 변하므로 역맵에 셀을
#   더할 때 나는 한글 잠식 위험이 없다(전례: 기호 10종이 501쪽·42.5% 를 깼다).
_ROMAN_UNIT_REV = {
    "⠴⠉⠍⠲": "cm", "⠴⠅⠛⠲": "kg", "⠴⠅⠍⠲": "km", "⠴⠍⠛⠲": "mg",
    "⠴⠙⠇⠲": "dL", "⠴⠍⠇⠲": "mL", "⠴⠓⠁⠲": "ha", "⠴⠍⠍⠲": "mm",
    "⠴⠍⠘⠼⠃": "m^2", "⠴⠉⠍⠘⠼⠃": "cm^2", "⠴⠅⠍⠘⠆": "km^2",
...
# ── 제71항 [다만] — 한글과 혼동되는 기호는 로마자표·종료표로 감싼다. 원장 R-28 ──
# "한글과 혼동되는 앰퍼샌드(&), 섹션 기호(§), 문단 기호(¶), 저작권 기호(©), 등록 상표(®),
#  상표 기호(™)는 로마자표와 로마자 종료표를 사용하여 적는다." (규정 예: `0@&4` = `&`)
# symbol_table 에는 **감싸지 않은** 두 셀 꼴만 있어 역맵이 감싼 꼴을 못 읽었다 —
# `& 클레이아트` 가 `”굴. 클레이아트`, `®는` 이 `”배.는` 으로 나갔다.
# ★ 감싼 꼴은 넣어도 안전하다. 두 셀 꼴 `⠘⠉`(©)·`⠈⠯`(&)는 한글 음절과 겹쳐 2026-09-03
#   에 본문 경로에서 뺐지만(위 `_SYMBOL_REV.pop` 목록), 여기 키는 앞뒤가 ⠴…⠲ 로 묶인
#   네 셀이라 그 충돌이 안 난다.
# 실측(전권 18,892쪽): & 147회·97쪽 · © 66회·4쪽 · §·¶·®·™ 0회.
_FENCED_SYM_REV = {"⠴⠈⠯⠲": "&", "⠴⠘⠎⠲": "§", "⠴⠘⠏⠲": "¶",
                   "⠴⠘⠉⠲": "©", "⠴⠘⠗⠲": "®", "⠴⠘⠞⠲": "™"}
# ⠲ 로 끝나는 시퀀스는 _decode_line 이 **_SYMBOL_REV 로** 마침표 분리 여부를 가른다.
# 두 맵을 같이 갱신해야 실제 출력이 바뀐다(_COMBINED 는 위에서 이미 복사됐다).
for _m in (_SYMBOL_REV, _COMBINED):
    _m.update(_ROMAN_UNIT_REV)
    _m.update(_FENCED_SYM_REV)
...
# 완결되면 로마자 구간의 시작이 아니다(아래 `_merge_roman_tokens` 참조).
# ★ 홑 ⠴ 는 넣지 않는다 — 그건 낱말 앞에 홀로 선 **로마자표**다(`⠴ largely
#   unjustified`). 넣었더니 영어 지문 한 줄이 통째로 한글로 깨졌다(실측 1쪽 −0.77p).
...
def _decode_roman_run(s: str, i: int, *, span_ok: bool = False) -> tuple[str, int] | None:
    ...
    while j < n:
        ...
        if (s[j:j + 2] == _UNIT_SLASH and s[i] == _ROMAN_START
            ...
            # 종료표가 앞에 있을 때만 이어 간다(공백·쉼표 처리와 같은 증거 조건).
            # 실측(전권 18,892쪽): 53회·36쪽. 원장 R-27.
            out.append("/")
            j += 2
            continue
        if c == _SUPERSCRIPT and s[i] == _ROMAN_START:
            ...
            if nxt in ("⠢", "⠔"):
                ...
                continue
            break                                    # 모르는 위첨자 → 런 종료
        if c == _SUBSCRIPT:                          # 아래첨자표 — 규정 제68항
            # ★ 다만 **닫는 대괄호 ⠰⠴** 는 건너뛰면 안 된다 — ⠰ 만 먹고 남은 ⠴ 가
            #   닫는 큰따옴표로 떨어져 `[A]` 가 `[A”` 로 나갔다. 진짜 첨자는 뒤에
            #   수표·글자가 오므로(`A_1`=⠴⠠⠁⠰⠼⠁) 이 조건에 안 걸린다.
            if s[j + 1:j + 2] == _ROMAN_START:
                break
            # ★ 제68항 "아래 첨자는 ; 뒤에 첨자의 내용을 적어" — `;`가 ⠰ 다.
            #   규정 예문 `B₆` = `0,b;#f`(⠴⠠⠃⠰⠼⠋). 위첨자(바로 위 가지)는 `^`를
            #   내는데 아래첨자는 표를 **버리고** 있었다. 그래서 로마자표로 열린 런
            #   안에서만 `O₂`가 `O2`로, `H₂O`가 `H2시`로 나갔다(수식 경로로 가는
            #   `d_1`(⠙⠰⠼⠁)은 멀쩡했다 — 이 가지를 안 타기 때문이다).
            #   위첨자 가지와 같은 조건으로 **로마자표로 연 런에서만** 본다.
            if s[i] == _ROMAN_START and s[j + 1:j + 2] == _NUMBER_SIGN:
                num, j = _decode_number(s, j + 1)
                out.append("_" + num)
                # ★ 첨자 숫자 **뒤는 대문자표일 때만** 런을 잇는다. 제68항 [붙임 1] 이
                #   "숫자 첨자 뒤에 …한글이 붙어 나올 때" 를 따로 정할 만큼 한글이 바로
                #   붙는다. 무조건 이으면 그 조사를 로마자로 먹는다 —
                #   실측: `O₂와 CO₂가` → `O_2v CO_2e` · `E₁일` → `E_1o,`.
...
def _korean_tail(tok: str, at: int) -> str | None:
    ...
    if len(rest) < 2:
        return None
    try:
        # sep=False — 구분표는 **읽는 법**이지 한글이라는 증거가 아니다. 여기서 떼면
        # 수식 꼬리 `⠉⠤⠌⠉`(c-분의c)가 `나예나` 로 읽혀 한글로 오판된다(실측 4건).
        d = _decode_line(rest, sep=False)
    except Exception:                                # noqa: BLE001
        return None
...
def _classify_token(tok: str) -> str:
    ...
    if (_MATH_REL_RE.search(tok) or _MATH_GE_HEAD_RE.match(tok)) and len(_symbol_cover(tok)) < len(tok):
        return "MATH"
...
def _merge_roman_tokens(tokens: list[str], seps: list[str]) -> tuple[list[str], list[str]]:
    ...
    while i < len(tokens):
        ...
        # ★ 등록된 **단위 기호 하나로 완결된 토큰**은 병합하지 않는다(제69항 [붙임2]).
        #   [붙임2]는 비로마자 단위를 `단위표 ⠴ + 낱자` 로 적고 **뒤에 한글이 오면 한 칸
        #   띄라**고 정한다(`1 %p다.` = `#a 0pp i4`). 그 한 칸 너머의 ⠲ 는 종료표가 아니라
...
def _join_num_hangul(text: str) -> str:
    ...
    # ★ 아래첨자 `_`도 같다 — `SiO_2 함량`의 한 칸은 낱말 사이다. `_`를 안 넣으면
    #   첨자를 복원하자마자 로마자 가드가 풀려 `SiO_2함량`으로 붙었다(실측 10자리).
    return re.sub(r"(?<![A-Za-z_])(\d) ([가-힣])", _repl, text)
...
# ── 구분표 (규정 제11·12항) ──────────────────────────────────────────────────
# 제11항 모음자에 '예'가 붙어 나올 때, 제12항 'ㅑ·ㅘ·ㅜ·ㅝ'에 '애'가 붙어 나올 때
# 두 모음자 사이에 구분표 `-`(⠤)를 적는다. **묵자에는 대응 문자가 없는 읽기 보조표**다
# (규정 예시: 아예 `<-/` · 서예 `,s-/` · 수액 `,m-ra` · 소화액 `,ujv-ra`).
# 역맵이 붙임표로 흘려보내 `인의예지`가 `인의-예지`, `수액`이 `수-액`이 됐다.
#
# 실측(전 코퍼스 18,892쪽): 제11항 **1,803건** · 제12항 **103건**.
# 묵자 대조(재추출 1,180쪽): 규정형 79건 전부 묵자에 붙임표가 **없다**(있음 **0건**).
#
# ★ **규정이 정한 앞 셀에서만** 뗀다. 조건을 풀면 묵자에 진짜 붙임표가 있는 자리를
#   같이 먹는다 — 제11항 그밖 43건, 제12항 그밖 161건(`고체-액체` 21건이 최다인데
#   ㅔ+애라 제12항이 아니다. 나머지는 수식 `a-분의a` 와 깨진 영어 줄이다).
#   앞 셀은 모음자 17종에 **약자 가·나·다·마·바·사·자·카·타·파·하**(제13항, 전부
#   ㅏ로 끝나 제11항의 '모음자'다)를 더한다 — `얼마예요`·`국가예요` 67건이 여기다.
# ★ 감쌈 붙임표 `⠤X⠤`(도서 관행 `(가)` → ⠤가⠤, _restore_wrap_parens 담당)와 셀이
#   겹친다. 그 자리는 **저쪽 규칙에 넘긴다** — 앞뒤 3셀 안에 다른 ⠤ 가 있으면 비켜난다.
#   실측: 가드가 막는 것 39건(2%)인데, 안 막으면 `들리시소(예).` 가 `들리시소예-.` 로
#   깨진다(전 코퍼스 4쪽).
# ★ **한글 경로(_decode_line)에서만** 뗀다. 수식·로마자 토큰에서 ⠤ 는 빼기표·붙임표이고
#   ⠏·⠗ 는 영문 p·r 이다 — 줄 선처리에 두면 `kpop-radar` 가 `k워이워radar` 로,
#   `c-분의c` 가 `c예나` 로 깨진다(실측 2쪽·5건. 두 토큰 다 is_math=True 다).
_SEP_MARK_RE = re.compile(
    r"(?<!⠤.)(?<!⠤..)(?<!⠤...)"                                  # ⠤X⠤ 의 닫는 쪽이면 비켜난다
    r"(?:(?<=[⠣⠗⠜⠎⠝⠱⠌⠥⠧⠽⠬⠍⠏⠩⠪⠺⠕⠫⠉⠊⠑⠘⠇⠨⠋⠓⠙⠚])⠤(?=⠌)"   # 제11항 모음자 + 예
    r"|(?<=[⠜⠧⠍⠏])⠤(?=⠗))"                                      # 제12항 ㅑㅘㅜㅝ + 애
