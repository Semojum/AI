...
_FLOORS: dict[str, dict[str, float]] = {
    ...
    # ★ 2026-09-03 하향(0.95 → 0.91): **본문에서 안 읽기로 정한 기호**가 늘었다.
    #   · 가역 화살표 ⇄(⠪⠶⠕)는 한글 `응이` 와 같은 셀 — 실측 404건 중 묵자에 ⇄ 0건.
    #   · 그리스 소문자 **규정 판본**(⠨+자음)은 book 모드가 안 내는데 전부 흔한 한글이다
    #     (⠨⠍=자우 10,183건 · ⠨⠹=자억 8,539 — 묵자에 그리스 0건). 수식 경로에는 남긴다.
    #   둘 다 **본문에서 읽으면 오검출**이라 뺐고, 그만큼 이 데이터셋의 단독 왕복은 준다.
    #   근거는 실물 1,180쪽 A/B — ⇄ 31:2 · 그리스 174:9. 회귀 감지 기능은 그대로다.
    # ★ 2026-09-03 재하향(build 0.91 → 0.89 · test 0.95 → 0.93): 같은 이유로 본문 역맵에서
    #   뺀 기호가 늘었다(∋ ∌ ∘ ∈ ⊕ ≡ ↓ © Å ◎). 이 데이터셋의 단독 왕복이 네 쌍 준다
    #   (build ©·Å · test ↓·∋). 근거는 **묵자 재추출 전수 실측**이다 —
    #   `!` 2,596회/389쪽 대 ∈ ∋ ≡ ↓ © Å ∘ **전부 0회**(⊕ 3회·◎ 31회).
    #   즉 이 셀들을 기호로 읽으면 그 자리는 사실상 항상 틀린다.
    "symbols": {"build": 0.89, "test": 0.93},
