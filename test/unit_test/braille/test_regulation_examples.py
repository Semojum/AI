...
class TestBracketRegulation:
    ...
    def test_수식_대괄호는_별도_점형(self) -> None:
        ...
        # ★ 안쪽 나열 쉼표는 `"`(⠐)다 — 종전 기대값 `#c1e`(⠂=자릿점)는 규정에 어긋났다.
        #   「수학 점자」 제1항 [붙임](재추출 3009행) "수의 **세 자리마다** 표기되어 있는
        #   쉼표는 1으로 적는다" / 제6항 예시(3103행) `A={2,4,6,...}` = `,a337#b"`#d"`#f"`,,,7`
        #   — 세 자리 묶음이 아닌 나열 쉼표는 "이고 뒤 수에 수표를 다시 적는다(2026-09-10).
        assert self._brf("<!수식>[3,5]<!/수식>") == "('#c\"#e,)"
