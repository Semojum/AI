...
# ── 나열 쉼표 ≠ 자릿점 (2026-09-10, 원장 M-09) ──────────────────────────────
# 「수학 점자」 제1항 [붙임](규정 재추출 3009행) "수의 **세 자리마다** 표기되어 있는
# 쉼표는 1으로 적는다" / 제6항 예시(3103행) `A={2,4,6,...}` = `,a337#b"`#d"`#f"`,,,7`.
# 종전에는 나열 쉼표까지 자릿점 ⠂로 접어 `{1,2,3}`이 한 수 `⠼⠁⠂⠃⠂⠉`가 됐다 —
# 점자만 보면 그럴듯한 수라 점역사가 못 찾는 오류였다.
@pytest.mark.parametrize("latex,expect", [
    (r"\{1,2,3\}", "⠶⠼⠁⠐⠼⠃⠐⠼⠉⠶"),   # 나열 → 쉼표 ⠐ + 수표 재기입
    ("(-6,0)",       "⠦⠔⠼⠋⠐⠼⠚⠴"),      # 좌표도 나열이다
    ("1,000",        "⠼⠁⠂⠚⠚⠚"),         # 세 자리 묶음 → 자릿점 ⠂ (그대로)
    ("10,000,000",   "⠼⠁⠚⠂⠚⠚⠚⠂⠚⠚⠚"),
])
def test_나열_쉼표와_자릿점을_가른다(latex, expect):
    assert convert_latex(latex) == expect
