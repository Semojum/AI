...
REV_CASES = [
    ...
    # [붙임2](재추출본 2719~2721행) — "그 뒤에 한글이 나오면 한 칸 띄어 쓴다".
    # 그 한 칸은 규정이 넣은 것이라 묵자에는 없다. 기대값은 규정 원문 묵자 줄이다
    # (2756·2758·2760행), 우리 출력이 아니다.
    ("69-rev-pct-e", "#aj0p`n", "10%에"),               # 연 강수량의 10%에 해당한다.
    ("69-rev-deg", "#ij0d`oi4", "90°이다."),            # 직각은 90°이다.
    ("69-rev-degc", "#ajj`0d,c`n,s", "100 ℃에서"),      # 물은 100 ℃에서 끓는다.
    ("69-rev-pct-wa", "#e`0p`v`#f`0p`w", "5 %와 6 %의"),  # 5 %와 6 %의 차이는 …
...
def test_단위표_뒤_한_칸은_조사일_때만_지운다() -> None:
    """가드 — 규정은 낱말 사이에도 같은 한 칸을 넣으므로 점자만으로는 못 가른다.

    재추출 묵자 실측(단위 뒤 한글 102곳): 조사면 붙이고(64) 낱말이면 띄는(30) 것이
    묵자 관행이다. 아래 셀열은 코퍼스 실물(EBS-E26-002 ans p0081)이고, 그 쪽의
    재추출 묵자가 `70% 이상이` 로 **띄어 적는다** — 여기서 칸을 지우면 안 된다.
    """
    assert "% 이상이" in decode("⠼⠛⠚⠴⠏⠀⠕⠇⠶⠕")
