"""캡션 전용 가드 — 없는 그림의 설명을 내보내지 않는다 (노션 Review, 2026-08-23).
...
# ── 가드5(AI 말투 줄 걷기)는 **관문으로 옮겼다** — 2026-09-08 재구조화 2단계 ──────────
# 그 사슬은 이제 캡션뿐 아니라 시각 초안 폴백·그림 회수·표 점역자주도 지난다. 캡션 한
# 자리에서만 재면 나머지 다섯 자리가 안 걸린 채 통과한다.
#   → test/unit_test/braille/test_llm_text_guard.py
#
# 여기 남은 것은 **캡션 전용**이다:
#   · `_reject_read_text` — '지면의 글자를 읽어 온 캡션' 판정. 캡션 단위에서만 뜻이 있다
#     (본문 요소에서 `본문:` 말머리는 정상이다).
#   · `_is_blank_crop`   — 크롭 이미지 판정. 글이 아니라 그림을 본다.
