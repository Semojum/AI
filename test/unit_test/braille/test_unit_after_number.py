...
@pytest.mark.parametrize("src,want", [
    # 「한글 점자」 제69항 [붙임 2](규정_텍스트.txt 2739~2741행) — 비로마자 단위 뒤에
    # 한글이 나오면 한 칸. 정답 도서 실물 셀: 생명과학 I ans p0045 `⠼⠙⠚⠴⠏⠀⠕⠊⠲`(40% 이다).
    ("40%이다.", "⠼⠙⠚⠴⠏⠀⠕⠊⠲"),
    ("12℃이다.", "⠼⠁⠃⠴⠙⠠⠉⠀⠕⠊⠲"),
    ("30°이다", "⠼⠉⠚⠴⠙⠀⠕⠊"),
    # 제65항 [붙임](2637행) 예문 2641행 `$1는 100￠이다.` = ``#ajj0@c`oi4``
    ("100￠이다.", "⠼⠁⠚⠚⠴⠈⠉⠀⠕⠊⠲"),
])
def test_단위_화폐_뒤_한글_앞은_한_칸(src, want):
    assert T(src) == want


@pytest.mark.parametrize("src", ["10% 이상", "각 90°와", "50%", "1m는"])
def test_한_칸_규칙이_다른_자리를_안_건드린다(src):
    """묵자에 이미 칸이 있거나 뒤가 한글이 아니면 그대로다(칸 둘로 안 벌어진다).

    `$…$` LaTeX 경계는 제외한다 — 그 자리 두 칸은 「수학 점자」 제11항 수식 경계라
    이 규칙과 무관하고 종전부터 그랬다.
    """
    assert "⠀⠀" not in T(src)
