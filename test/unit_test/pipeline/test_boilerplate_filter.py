...
class TestIsBoilerplate:
    ...
    def test_circled_c_marker_kept(self):
        # ⓒ(U+24D2)는 보기 표시 문자(ⓐⓑⓒⓓ)이기도 하다. 저작권 고지로 오인해 요소를
        # 통째로 버리면 점자에서 보기 항목 하나가 사라진다(#661).
        # 아래는 코퍼스 1,180쪽에서 실제로 버려졌던 다섯 요소다 — 다섯 다 정답 점자책에 있다.
        for c in [
            "ⓒ구보는 맞은편에 앉은 사내의, 그 교양 없는 이야기에 건성 맞장구를 치며,"
            " 언제든 그러한 것을 연구하여 보리라 생각한다.",
            "ⓒ많이 주면 무사하고 적게 주면 트집 잡네",
            "ⓒ에서 주어로 '영희는' 대신 '나는'이 오기 어려운 이유는 '떠나'는 것이"
            " 미래의 사건이기 때문이 아니고,",
            "ⓒ에서는 관람에 필요한 정보를 제공하여 전시를 홍보하기 위해",
            "ⓒ의 '국민께'에 쓰인 조사 '께'는 부사어가 지시하는 대상인 '국민'을 높이고 있지만,",
        ]:
            assert not _is_boilerplate(c), c
        # 발행처·고지 문구가 같이 오면 그대로 고지다
        for c in [
            "ⓒ EBS 한국교육방송공사",
            "ⓒ 2026 한국교육방송공사. All rights reserved.",
        ]:
            assert _is_boilerplate(c), c
