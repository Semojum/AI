...
태깅 LLM이 상자 제목을 `<!테두리_위>` 안에 넣을 때와 본문 줄로 남길 때가 갈린다. 원인은
...
class TestPromote:
    def test_끝줄_보기를_올린다(self) -> None:
        ...
        assert got.startswith("<!테두리_위>보기<!/테두리_위>")
        assert "\n보기\n" not in got          # 본문에서는 빠져야 한다

    def test_첫줄_보기도_올린다(self) -> None:
        src = f"<!테두리_위><!/테두리_위>\n보기\n{_ITEMS}\n<!테두리_아래><!/테두리_아래>"
        assert _promote_box_title(src).startswith("<!테두리_위>보기<!/테두리_위>")

    def test_이미_제목이_있으면_그대로(self) -> None:
        src = f"<!테두리_위>보기<!/테두리_위>\n{_ITEMS}\n<!테두리_아래><!/테두리_아래>"
    ...
    def test_2단계_테두리도_같이_본다(self) -> None:
        src = f"<!테두리_위2><!/테두리_위2>\n{_ITEMS}\n보기\n<!테두리_아래2><!/테두리_아래2>"
        assert _promote_box_title(src).startswith("<!테두리_위2>보기<!/테두리_위2>")
