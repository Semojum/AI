...
def test_그림_사진_그래프는_분량과_형식_둘로_갈린다():
    """분량 2단(§6.1.4(4) 윤곽/부분) × 형식 2가지(§6.1.4(6) 개조식 · (7) 진술).

    ★ 2026-09-07 — 종전에는 `prose_label` 이 '설명(자세히)' 를 돌려주고 그 안의 내용을
      LLM `[줄글]` 절로 채웠다. 이름과 내용이 거꾸로였다(실측: '설명' 4줄 vs
      '설명(자세히)' 1줄, 게다가 캡션에 없는 문장).
    ★ 2026-09-09(#793) — 줄글을 **되살렸다.** 다른 점은 재료다. 그때는 LLM 이 새로 지은
      문장이었고 지금은 **설명 안이 쓰는 그 줄들을 이어 붙인 것**이라 새 말이 못 들어간다.
