...
def test_캡션_줄이_개조식_항목이_된다():
    ...
    # 점역자 주표는 유형 낱말만 감싸고 쌍점부터는 밖이다(원장 C-D4, 도서지침 L2368).
    assert lines[0].startswith("<!2칸><!주>그래프<!/주>: 시간에 따른 개체 수"), text
...
def test_사진_캡션은_사진으로_나간다():
    ...
    assert _desc(ImageOpt, "사진: 파르테논 신전") == "<!2칸><!주>사진<!/주>: 파르테논 신전"
    assert _desc(ImageOpt, "그림: 세포 모형") == "<!2칸><!주>그림<!/주>: 세포 모형"
