...
class TestCaptionMapping:
    ...
    def test_완료_순서가_뒤섞여도_요소별_캡션이_맞다(self) -> None:
        ...
        def fake(el, context=""):
            ...
            return f"CAP-{el['element_id']}", el["type"], True, None, "", False
    ...
    def test_읽기_순서가_보존된다(self) -> None:
        with patch.object(rb, "_do_caption",
                          side_effect=lambda el, context="": (f"C{el['element_id']}", el["type"], True, None, "", False)):
    ...
    def test_텍스트_요소는_캡셔닝하지_않는다(self) -> None:
        ...
        with patch.object(rb, "_do_caption",
                          side_effect=lambda el, context="": ("CAP", el["type"], True, None, "", False)) as m:
            out = _build(els)
...
class TestIsolation:
    ...
    def test_캡셔닝_실패_요소도_남는다(self) -> None:
        ...
        with patch.object(rb, "_do_caption",
                          side_effect=lambda el, context="": ("", el["type"], False, None, "", False)):
            out = _build([_el(0)])
...
class TestCallCount:
    ...
    def test_동시_1로_제한해도_결과가_같다(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        side = lambda el, context="": (f"CAP-{el['element_id']}", el["type"], True, None, "", False)  # noqa: E731
...
def test_동시_처리가_직렬보다_빠르다() -> None:
    ...
    delay = 0.05
    side = lambda el: (time.sleep(delay), ("CAP", el["type"], True, None, "", False))[1]  # noqa: E731
    n = 8
    with patch.object(rb, "_do_caption", side_effect=side):
        t0 = time.monotonic()
        _build([_el(i) for i in range(n)])
