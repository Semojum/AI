...
class TestTypeWordHead:
    """머리줄이 `그림:`·`사진:` 으로 열려도 제목이지 인물 대사가 아니다.

    §5.3.1(1) "'만화'를 점역자 주표를 사용하여 5칸에 표기하고, 한 칸 띈 후 만화 제목을
    적는다" · §5.3.3(5) 인물명은 **인물이 지닌 특징**이지 유형 제시어가 아니다.
    실측(테스트_이미지 5쪽): 캡션이 만화 형식을 안 지키고 `그림: 인생 시기별 모습 다섯 칸`
    으로 열어, 그 줄이 `그림` 이라는 인물의 대사로 실렸다.
    """
    ...
    _CAP = (
        ...
        "\n"
        "현재: 책가방을 멘 학생임.\n"
        "8년 후: 대학 건물 앞에서 그림을 그리고 있음.\n"
    )

    def test_유형_제시어_머리줄은_제목으로_간다(self):
        title, items = _caption_items(self._CAP)
        assert title == "인생 시기별 모습 다섯 칸"
        assert not any("그림:" in t for _, t in items)
        assert items[0][1] == "현재: 책가방을 멘 학생임."

    def test_만화_머리줄은_종전대로다(self):
        title, items = _caption_items("만화: 두 학생이 이야기하고 있음.\n남학생: 안녕?\n")
        assert title == "두 학생이 이야기하고 있음."
        assert items == [(0, "남학생: 안녕?")]
