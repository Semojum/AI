...
class TestOrgChart:
    def test_위계_전사(self):
        ...
        assert lines[1] == "<!주>조직도<!/주>:"          # 유형(도서지침 예3-46)
        # 정본 자료지침 예6-22가 쓰는 말 — **칸 수를 밝힌다.** 점자에는 선·상자가 없어
        # 위계가 들여쓰기로만 남는데, 몇 칸이 한 단계인지 모르면 빈칸을 세도 단계를 못 센다.
        assert lines[2] == "<!주>하위에 속한 기구를 2칸씩 들여 쓰기함<!/주>"
...
class TestFamilyTree:
    def test_하향식_트리(self):
        ...
        lines = text.split("\n")
        assert lines[0] == "<!주>가계도<!/주>:"
        assert lines[1] == "<!주>선조에서 후손 순(하향식)<!/주>"
    ...
    def test_상향식_평면_3칸(self):
        ...
        lines = text.split("\n")
        assert lines[0] == "<!주>가계도<!/주>:"
        assert lines[1] == "<!주>후손에서 선조 순(상향식)<!/주>"
...
class TestForm:
    def test_글상자_항목_노트(self):
        ...
        assert lines[1] == "<!주>양식<!/주>:"
        assert lines[2] == "<!상자><!/상자>"                     # §6.6.3(2) 글상자
        assert lines[3] == "이름:" and lines[4] == "주소:"                   # §6.6.3(3) 한 줄에 하나
        assert lines[5] == "<!주>긴 빈칸 두 줄<!/주>"           # §6.6.3(5)
        assert lines[6] == "<!상자끝><!/상자끝>"
...
class TestScreenImage:
    def test_글상자_구획(self):
        ...
        lines = text.split("\n")
        assert lines[0] == "<!주>화면 이미지<!/주>:"
        assert lines[1] == "<!상자><!/상자>"                     # §6.6.7(1)
