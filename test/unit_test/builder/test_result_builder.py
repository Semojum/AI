...
def test_image_captioning_result(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda img_path, img_type, context="", out_info=None: "테스트 캡셔닝: 두 학생이 실험 도구를 사용하는 장면입니다.",
    )
    monkeypatch.setattr(
...
class TestEmptyCaptionKeepsElement:
    ...
    def test_빈_캡션이어도_요소가_남는다(self, monkeypatch):
        ...
        monkeypatch.setattr(rb, "caption", lambda p, t, context="", out_info=None: "   ")
    ...
    def test_정상_캡션은_그대로(self, monkeypatch):
        import app.ai.builder.result_builder as rb
        monkeypatch.setattr(rb, "classify_with_confidence", lambda p: ("cartoon", 0.9, ""))
        monkeypatch.setattr(rb, "caption", lambda p, t, context="", out_info=None: "만화: 후보 토론회")
        monkeypatch.setattr(rb.Path, "exists", lambda self: True)
