...
# ── 문장부호로 시작하는 이어짐 (2026-08-27) ──────────────────────────────────
# 글자로만 이어짐을 보면 여는 괄호·따옴표로 이어지는 줄이 통째로 막힌다.
# 전수 실측(코퍼스 경계 파일 2,801쪽, 요소 안쪽 개행 52,392개): 막힌 2,309개 중
# 첫 글자가 `(` 639 · `…` 208 · `‘` 163 · `[` 157 · `,` 56 · `“` 43.
# 규정 근거: 「점자 자료 제작 지침」 §2.4.11 — 인쇄 줄 경계는 강제 줄바꿈 신호가 아니다.

@pytest.mark.parametrize("head", ["(casta)’에서 유래한 신분 제도로서 서로 다른",
                                  "‘카스타’에서 온 말로 폐쇄성이 매우 강하다는",
                                  "…이어지는 말이 여기서 계속된다고 볼 수 있다",
                                  ")회 일어난다고 보아야 맞는 설명이 된다",
                                  ", 그리고 그 뒤에 이어지는 설명이 계속된다"])
def test_문장부호로_시작해도_이어진다(head):
    """시연 p02 실물: "…포르투갈어 ‘카스타" ⏎ "(casta)’에서 유래한…"."""
    els = [_line(100, "카스트는 순수한 혈통을 가리키는 포르투갈어 ‘카스타"),
           _line(115, head)]
    assert len(_join(els)) == 1


@pytest.mark.parametrize("head", ["-3 항목이 이어지는 것처럼 보이지만 다른 항목이다",
                                  "*표는 새 항목을 여는 글머리표로 쓰인다",
                                  "=값 은 표 안 수식 조각이라 이으면 안 된다"])
def test_글머리표나_잔재로_시작하면_안_잇는다(head):
    """`-`·`*`·`=`는 글머리표거나 추출 잔재다 — 이으면 다른 항목이 붙는다."""
    els = [_line(100, "앞 줄은 문장이 끝나지 않은 채로 단 끝까지 차 있다고 하자"),
           _line(115, head)]
    assert len(_join(els)) == 2
